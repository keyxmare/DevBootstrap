...
class CommandRunner:
    ...
    def copy_file(self, src: str, dst: str) -> bool:
        ...
        try:
            if self.dry_run:
                self.cli.print_info(f"[DRY RUN] cp {src} {dst}")
                return True

            shutil.copy2(src, dst)
            return True
        except Exception as e:
            self.cli.print_error(f"Impossible de copier {src} vers {dst}: {e}")
            return False
    ...
    def copy_directory(self, src: str, dst: str) -> bool:
        ...
        try:
            if self.dry_run:
                self.cli.print_info(f"[DRY RUN] cp -r {src} {dst}")
                return True
    ...
    def remove(self, path: str) -> bool:
        ...
        try:
            if self.dry_run:
                self.cli.print_info(f"[DRY RUN] rm -rf {path}")
                return True
            ...
            self.cli.print_error(f"Impossible de supprimer {path}: {e}")
            return False
    ...
    def clone_git_repo(
        ...
        if self.dry_run:
            self.cli.print_info(f"[DRY RUN] git clone --depth {depth} {url} {destination}")
            return True
        ...
        result = self.run(
            ["git", "clone", "--depth", str(depth), url, destination],
            sudo=False,
            timeout=300
        )
